...
const viewDetails = async (item) => {
  ...
  get(`api/answer/getMyAnswerDetails/${item.id}`, data => {
...
const getAnswerMultiple = (questionId) => {
...
const getAnswerSingle = (questionId) => {
  const answer = myAnswerList.value.find(item => item.questionnaireDetailsId === questionId);
  if(answer)
    return [answer.myanswer];
  return [];
...
<template>
  <div style="display: flex">
    <div class="settings-left">
      <card  title="我填写的问卷" :icon="Avatar" desc="在这里查看历史填写的问卷调查" v-if="!isViewingDetails">
      ...
      <card :title="currentQuestionnaire.name" :icon="Avatar" :desc="currentQuestionnaire.desc" v-if="isViewingDetails">
        <div v-for="(question, index) in currentQuestionnaireDetails" :key="index">
          ...
          <template v-if="question.type === 0"> <!-- 单选题 -->
            <el-radio-group class="questionnaire-details-item" :v-model="question.answer" :model-value="getAnswerSingle(question.id)[0]" disabled>
          ...
          <template v-else-if="question.type === 1"> <!-- 多选题 -->
            <el-checkbox-group class="questionnaire-details-item" :model-value="getAnswerMultiple(question.id)" :v-model="question.answer" disabled>
          ...
          <template v-else-if="question.type === 2"> <!-- 文本题 -->
            <el-input placeholder="请输入文本" :v-model="question.answer" :model-value="getAnswerSingle(question.id)" disabled></el-input>
          </template>
        </div>
